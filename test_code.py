...
import sys
sys.path.append('/share/drought-3/intern/jwlee/LAI/test/')
import user_inputs as inp

path = '/share/drought-3/intern/jwlee/LAI/test/'
...
csvfile = open(path + "points_coordinates.csv", newline='')
...
for point in all_points:
    ...
    for var in lookedup_tiles:
        try:
            if "MSIL2A" in var['Name']: # 요 MSIL2A 데이터 말고 MSIL1C 데이터를 다운 받아야함, 근데 동일한 이름인지 확신이 없음
                ...
                with open(os.path.join(path, f""+var['Name']+".zip"), 'wb') as p:
                    p.write(file.content)
        except:
            pass
    count = count + 1
## 모르는 함수 : .Session(), headers, update, Bearer, .split(), .get, 
## ## ## url을 모두 뽑아낸 뒤, 리눅스에서 wget 함수로 다운로드 
